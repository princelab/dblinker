...
class Array
      ...
      #
      def search_lower_boundary(range=nil, &block)
...
# Ryan's fxn for finding the window
	def bin_window(array, i, point)
		range = nil; 
		low = point - Cut_off
		up = point + Cut_off
		lower = search_lower_boundary(array, range, "{|x| x[#{i}] <=> #{low}}")
		upper = search_upper_boundary(array, range, "{|x| x[#{i}] <=> #{up}}")
		return lower ... upper
	end
